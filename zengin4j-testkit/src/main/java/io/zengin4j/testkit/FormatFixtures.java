 ...
 *
 * <p><strong>Every value produced through this interface is invented</strong>
 * (R-L1, P1). Bank {@code 9999}, branch {@code 999} and accounts beginning with
 * {@code 9} are outside the ranges Japanese institutions use; names come from a
 * fixed list of obviously fictional katakana.
 *
 * <p>The four bundled formats do not share a record layout, and this interface
 * deliberately does not pretend they do. What it shares is the <em>shape of the
 * question</em> — give me a header, some payments, a trailer that adds up —
 ...
 * write.
 *
 * <p>The direction of a payment is likewise the implementation's business:
 * 預金口座振替 debits the accounts its data records name, where 総合振込 credits
 * them. {@link #data(String, long, String)} names a counterparty, and which side
...
public interface FormatFixtures {
     ...
     *
     * <p>Includes {@code allowUnverifiedFormats(true)}: every bundled
     * descriptor is provisional, and the reader refuses one otherwise.
     *
     * @return reader options, never {@code null}
     */
