 ...
 *
 * <p>One class serves both formats because the standard says they are one
 * layout under two 種別コード — the same reason {@code shoyo-furikomi.yaml}
 * borrows its fields rather than restating them. {@link #kyuyo()} and
 * {@link #shoyo()} differ only in which descriptor they encode against.
 *
 * <p><strong>This is not 総合振込 with three fields renamed.</strong> The data
 ...
 *
 * <p>Names go through the {@code PAYROLL_NAME} character class, which admits no
...
public final class KyuyoFurikomiFixtures extends AbstractFormatFixtures {
    ...
    /** Employee number, 社員番号. Invented, and declared {@code C}; see D-002. */
