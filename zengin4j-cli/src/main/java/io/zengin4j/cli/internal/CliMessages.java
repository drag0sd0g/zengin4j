...
///
/// R-E3 asks every diagnostic to say how to fix the problem, and the library
/// answers that correctly for its own audience: a caller writing Java is told to
/// set `ReaderOptions.builder().allowUnverifiedFormats(true)`. Printed at a
...
///
/// So the remedy is translated on the way out. This is deliberately a small
/// lookup rather than a clever rewriter: string replacement is fragile, and the
/// fragility is contained by `NoJavaRemediesReachTheTerminalTest`, which
...
public final class CliMessages {
    ...
    ///
    /// Exposed so the guard test and this class agree on what counts, rather
    /// than each carrying its own idea of it.
    ///
    /// @param text the text to check
    /// @return `true` if it mentions a builder or options class
