...
### Changed — further Java 25 uplifts

- **The code generators build their output with text blocks.** 745 `.append()` calls became 301
  across the six generators; what remains is the per-field and per-row loops, which genuinely
  interpolate. Every generated byte is unchanged, which `checkGeneratedSources` proves on each run.
- **`var` where the right-hand side names the type** — 285 declarations: `new X(...)`,
  `X.of(...)`-style factories, and casts. Deliberately *not* applied where the initialiser is a
  method call, a diamond `new X<>()`, or a numeric local, on the grounds that a library about byte
  offsets should keep `int` and `long` visible. Recorded in
  [ADR-0037](docs/adr/0037-markdown-doc-comments-and-module-imports.md).
- **`RecordSourceGenerator`'s descriptor checks are switch expressions over `RecordKind`.** Only
  expressions are checked for exhaustiveness — an arrow-form switch *statement* over an enum is not,
  and `-Xlint:all` says nothing — so the obligations each record role imposes are now data returned
  from a switch, and adding a record kind fails this file to compile.
- **The YAML readers coerce with pattern switches**, including `case Number _, Boolean _` and a
  `when` guard for the negative-integer diagnostic.
- **`zengin diff` reports the first differing byte.** `Arrays.mismatch` answers "do these differ"
  and "where" in one pass, so a field change now reads
