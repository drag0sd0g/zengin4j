 ...
 * </pre>
 *
 * <p>An institution that publishes its own 総合振込 variant gets its own
 * {@code FormatId} (R-X1). The bundled mapping is keyed on
 * {@code sougou-furikomi}, so converting that variant fails until the rows are
 * registered under the new id — which is what this shows.
 *
 * <p><strong>Rows are a declaration, not a rule engine.</strong> Registering
 ...
 * mapping <em>claims</em> and not what it <em>does</em>. See
 * {@code docs/adr/0035-the-mapping-is-data-not-a-rule-engine.md}, and use
 * {@code MappingContext.endToEndPolicy} to redirect where {@code EndToEndId}
 * lands, which the mapper does act on.
 *
 * <p><strong>Every identifier here is invented</strong> (R-L1).
 */
public final class CustomMappingRegistry {
     ...
     *
     * <p>Real variants differ in a filler field or a narrowed code list. What
     * matters for the mapping is that the field <em>ids</em> are the standard
     * ones, because that is what the mapper reads fields by.
     *
     * <p>The records are rebuilt rather than borrowed: a {@code RecordDescriptor}
