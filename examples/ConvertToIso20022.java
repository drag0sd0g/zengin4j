...
/// ```
///
/// The point of this example is not that conversion works. It is that
/// conversion *costs* something, that the cost is returned rather than
...
/// Fixed rather than `LocalDate.now()`: it supplies the year that
/// `MMDD` fields lack, and it is the basis of `MsgId` and
/// `CreDtTm`, so pinning it makes the output reproducible.
...
/// Zengin to `pain.001`, and what it cost.
...
///
/// Here the reference is thirty-five characters and 顧客コード1 holds ten,
...
///
/// A ZEDI file carries the business application header and the message
