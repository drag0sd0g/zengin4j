 ...
 *
 * <p>Worth fuzzing for a reason the fixed-length reader is not: this one scans
 * for a byte sequence and cuts a file at every occurrence, so the interesting
 * inputs are the ones where {@code <?xml} lands somewhere structurally
 ...
 *
 * <p>Two promises are under test.
 *
 * <ul>
 *   <li><strong>Termination and typed failure.</strong> Any byte sequence
 *       either reads or raises a {@link ZenginException}. Anything else — a
 *       {@code StringIndexOutOfBoundsException}, a hang, an
 *       {@code OutOfMemoryError} — is a defect, because this reader's whole
 *       job is to survive a file somebody else produced.
 *   <li><strong>R-I6: what was read is written back byte for byte.</strong>
 *       Held for <em>every</em> input the reader accepts, not just for
