...
///
/// `BigDecimal` rather than `long`, although a Zengin amount is
/// always whole yen: the XML side is `DecimalNumber` and a sender is free
/// to write `150000.00`, or a non-JPY amount, or a fraction of a yen.
...
/// `COERCED` loss at `CRITICAL`, not a rounding nobody sees.
///
...
public record Money(BigDecimal amount, String currency) {
    ...
    /// could not be read.
    ///
    /// Not a placeholder: it is the standard's own way of saying that a figure
    /// is not a currency amount, and it flows through the same path a foreign
    /// currency does — reported `CRITICAL`, because a payment whose amount
    ...
    ///
    /// A guard against untrusted input, not a business limit. `xs:decimal`
    /// admits `1e2000000000`, which parses in microseconds and exhausts a
    /// heap the moment anything renders it — thirteen bytes of input for an
    /// `OutOfMemoryError`. Thirty digits is a thousand times the money in
    ...
    ///
    /// @param element the `Amt` element
    /// @return the amount, or empty when `InstdAmt` is absent, is not a
