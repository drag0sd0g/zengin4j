...
/**
 * A transaction's {@code RmtInf}, held as the lines it actually contains.
 *
 * <p>The profile uses unstructured remittance information for two unrelated
 * purposes: ordinary free text, and the base64-encoded 金融EDI payload
 * ({@link EdiAttachment}). Modelling it as the raw {@code Ustrd} list and
...
public record RemittanceInformation(List<String> unstructured) {
     ...
     *
     * @param element the {@code RmtInf} element
     * @return the remittance information
     */
     ...
     *
     * @return the {@code RmtInf} element, or empty when there is nothing to write
     */
