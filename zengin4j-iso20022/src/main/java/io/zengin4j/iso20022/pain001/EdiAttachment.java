...
/// information.
///
/// The profile does not put structured remittance data in the structured
...
/// ```
///
/// **The encoding is kept exactly as it arrived (R-I12).** Two
...
/// written back verbatim. [#decodedBytes()] decodes a copy for callers who
/// want to read it.
///
...
public record EdiAttachment(List<String> mimeHeaders, List<String> base64Lines) {
    ...
    /// Recognises an attachment in a run of `Ustrd` lines.
    ///
    /// Recognition is by the transfer-encoding header, not by position: a
    /// sender that omits `MIME-Version` or orders the headers differently
    ...
    /// The lines as they belong in `RmtInf`, headers first.
    ///
    /// @return the `Ustrd` contents, in order
