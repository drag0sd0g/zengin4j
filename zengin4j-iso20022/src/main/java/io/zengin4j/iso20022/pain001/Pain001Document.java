...
/// A `pain.001.001.03` customer credit transfer initiation.
///
/// A hand-written subset, not a generated binding. The profile uses a small
...
/// `docs/adr/0031-hand-written-iso20022-xml.md`.
///
/// What that costs: elements outside the subset are not modelled. They are
/// not lost, though — [#toXml()] and the reader work on
/// [XmlElement], so a caller who needs `Purp` or a postal address
/// can reach it through the tree.
///
/// @param groupHeader `GrpHdr`
/// @param payments    `PmtInf`, in order; at least one in a valid message
/// @since 0.5.0
public record Pain001Document(GroupHeader groupHeader, List<PaymentInstruction> payments) {
    ...
    ///
    /// @return the `Document` element, ready to serialise
