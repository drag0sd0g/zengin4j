...
///
/// ## Every conversion reports what it lost
///
/// There is no method here that returns a converted file on its own (R-I14).
...
/// `MMDD`, and JPY is the only currency one side can express — so every
/// conversion loses something, and an API that let a caller forget that would be
/// lying about the thing that matters most.
///
/// By default a conversion **refuses** when the loss reaches
/// [LossSeverity#CRITICAL]: money that could reach the wrong account is
/// not something to return quietly and hope somebody reads the report. Set
/// [MappingContext.Builder#acceptAnyLoss()] to get it back.
///
/// ## Example
///
/// ```java
...
public final class Iso20022Mapper {
    ...
    /// Converts a Zengin file to a `pain.001` message in its ZEDI envelope.
    ///
    ...
    ///
    /// The context is required and is not defaulted (R-I20): the XML does not
    ...
    ///   [io.zengin4j.iso20022.envelope.ZediEnvelopeReader]
    ...
    ///
    /// Serves the question "what would this cost me?", which is worth being
    /// able to ask before committing to an answer. It never refuses, whatever
    /// [MappingContext#failOnSeverity()] says — the whole point is to see
    ...
    /// legs (R-I18).
    ///
    /// The honest demonstration that conversion is not bijective. Run it on a
    ...
    /// fit in ten.
    ///
    /// Like [#dryRun], this does not refuse — a round trip that stopped
    ...
    /// The `pain.001` document inside a ZEDI file.
    ///
    /// R-I3 pins the version. A document declaring `pain.001.001.09` is
