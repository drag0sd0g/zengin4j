 ...
 *
 * <p>The mapper works through this rather than through the generated accessors,
 * so that a mapping row saying {@code header.originBankCode} is what actually
 * fetches the value. The alternative — casting to {@code SougouFurikomiHeader}
 * and calling {@code originBankCode()} — would compile whatever the declaration
 * said, which would make the declaration decoration rather than code.
 *
 * <p>The cost is that a typo in the declaration becomes a runtime lookup rather
...
final class ZenginFields {
     ...
     *
     * @param reference {@code header.originatorName} and the like
     * @return the row
     */
