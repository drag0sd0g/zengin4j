...
/// Where `EndToEndId` lives on the Zengin side.
///
/// ISO 20022 makes `EndToEndId` mandatory and gives it 35 characters:
...
///
/// So there is no correct answer, only a choice the caller has to make and
...
public enum EndToEndIdPolicy {
    ...
    /// Carry it in 顧客コード1.
    ///
    /// The natural home — the table in §15.9 puts it here — but ten bytes
    /// against thirty-five. A reference that does not fit is truncated and
    /// reported `CRITICAL`, because a truncated reconciliation reference
    ...
    /// Carry it in 顧客コード2.
    ///
    /// Same size, same truncation risk. Worth choosing when 顧客コード1 is
    /// already committed to something the originator's own systems key on.
    CUSTOMER_CODE_2,

    /// Do not carry it.
    ///
    /// Reported `DROPPED` at `MATERIAL` on the way down, and
    /// written as `NOTPROVIDED` on the way up — the value the standard
