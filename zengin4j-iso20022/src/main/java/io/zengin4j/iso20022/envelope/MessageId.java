 ...
 *
 * <p>{@code pain.001.001.03} names a message <em>and</em> a version, and the
 * two cannot be separated: the profile pins the version (R-I3), so a document
 * declaring {@code pain.001.001.09} is not a newer dialect of something this
 * library understands, it is a different message. Keeping the identifier whole
 * makes that impossible to blur.
 *
 * @param value the identifier, e.g. {@code pain.001.001.03}
 * @since 0.5.0
 */
public record MessageId(String value) {
    ...
    /**
     * The message name without its version — {@code pain.001} for
     * {@code pain.001.001.03}.
     *
