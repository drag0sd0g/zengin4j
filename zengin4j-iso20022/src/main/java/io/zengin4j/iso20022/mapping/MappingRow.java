 ...
 *
 * <p>Rows are data, generated from {@code zengin4j-iso20022/mappings/*.yaml}
 ...
 * documentation.
 *
 * <p>A row is not executable. It records what the mapper is supposed to do, and
 * a test checks that the mapper does exactly that and nothing else — every
 * element the mapper emits has a row, and every {@code to-iso} row produces an
 ...
 * runtime.
 *
 * @param zenginField  {@code header.originatorName} and the like, empty when
 *                     the element has no fixed-length source
 * @param isoPath      the path below {@code Document}, empty when the field is
 *                     dropped
...
public record MappingRow(
     ...
     *
     * @return {@code header}, {@code data} or {@code trailer}, or empty when
