...
///
/// Two reasons the text lives outside the code. A translation is only
...
/// *both* languages at once (R-V2), which means the code needs both at
/// the same moment; resolving one against the JVM locale, as a
/// [ResourceBundle] normally would, is the wrong shape.
///
/// So both bundles are loaded explicitly and a message is fetched from each.
/// The JVM locale still decides what [ValidationReport#toText()] renders,
/// but it does not decide what a finding contains.
///
/// A missing key is a build-time defect, not a runtime one: it throws, and
/// `MessageBundleTest` asserts that every rule id has an entry in both
/// bundles. Shipping a finding whose Japanese text says
/// `!V-301.message!` would be worse than shipping no Japanese at all.
///
/// @since 0.2.0
public final class Messages {
    ...
    ///
    /// `ResourceBundle.getBundle(name, ENGLISH)` would not do. Its
    /// lookup falls back to the *default* locale before the base bundle,
    ...
    ///
    /// Loading each file directly also matches what is actually wanted here:
    /// both languages at once (R-V2), not one chosen by the environment. These
    /// are still [PropertyResourceBundle]s read from properties files, so
    ...
    ///
    /// @param key       the message key, conventionally `<ruleId>.message`
    /// @param arguments values to substitute, in [MessageFormat] order
    ...
    /// @return `true` if both bundles have it
