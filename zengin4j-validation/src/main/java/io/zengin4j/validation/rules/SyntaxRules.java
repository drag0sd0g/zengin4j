...
/// `V-2xx`)
///
/// These run per field, over every record whose length and discriminator let
/// its fields be located — including records the reader rejected. A record is
/// usually malformed *because* one of its fields contains something it
...
public final class SyntaxRules {
    ...
    ///
    /// **Malformed records are included**, provided their length
    /// and discriminator are right. Skipping them would be exactly backwards: a
    /// record is usually malformed *because* a field contains something
    ...
    ///
    /// What is skipped is a record whose length is wrong or whose
    ...
    /// The layout a record's fields sit in, or `null` when it cannot be
    /// determined.
    ...
    /// What [#forEachField] hands a rule.
    ...
    /// `open: true`, meaning verification confirms the listed values rather
    ...
    /// kana take one. `ｱﾞ` is not a typo for anything; it is a byte pair
