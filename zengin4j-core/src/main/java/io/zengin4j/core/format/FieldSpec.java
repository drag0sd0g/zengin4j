...
///
/// This type exists so that **nothing outside this class ever writes a
/// byte offset** (R-F2). A caller states each field's length in layout
/// order and [RecordDescriptor#of] assigns the offsets from the
...
///
/// Hand-written offsets are the largest single source of defects in
...
public record FieldSpec(
    ...
    ///
    /// Declared per field rather than per format, because the standard
    /// narrows the permitted set by what the field *is*: a branch name
    ...
    ///
    /// 付録3 states the master list once and then says that not every code is
    ...
    ///
    /// `N` means digits, which is a character rule and not merely a
    /// hint. `C` defaults to unrestricted rather than to a name class:
