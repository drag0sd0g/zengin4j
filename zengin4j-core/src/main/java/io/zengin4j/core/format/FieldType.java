...
///
/// Both are fixed-width byte ranges. They differ in permitted content,
...
public enum FieldType {
    ...
    /// `'0'`. An omitted value is all zeros.
    N(Alignment.RIGHT, (byte) '0'),

    /// Half-width katakana, upper-case `A`-`Z`, digits and a
    /// limited symbol set, left aligned, padded on the right with a space. An
    /// omitted value is all spaces.
    ///
    /// Lower-case Latin and full-width characters are not permitted
    ...
    /// Parses a descriptor's `type` value.
    ///
    /// @param raw    the raw value, `N` or `C`
    /// @param origin the descriptor resource name, for diagnostics
    /// @return the field type
    /// @throws FormatDescriptorException if the value is not `N` or
    ///   `C`
