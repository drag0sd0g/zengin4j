...
/// The optional `format` attribute of a field descriptor: a closed
/// vocabulary of interpretations a field may carry beyond its raw bytes.
///
/// Interpretation is declared, never inferred. A generator that decided
/// "this `N(10)` field is called amount, so it must be a monetary value"
...
public enum FieldFormat {
    ...
    /// Four digits, `MMDD`, with no year component. Decoded to
    /// [java.time.MonthDay]; the year is never invented (R-D9).
    MMDD("MMDD", 4),

    /// A monetary amount in whole yen. Decoded to `long`: there is no
    /// minor unit, so `BigDecimal` would add nothing (R-D6).
    AMOUNT("AMOUNT", 0),

    /// A record count. Decoded to `int`.
    COUNT("COUNT", 0),

    /// コード区分, the encoding indicator. Decoded to
    /// [io.zengin4j.core.charset.CodeKubun].
    ...
    /// Parses a descriptor's `format` value.
    ///
