...
public enum RecordKind {
    ...
    /** データレコード — データ区分 {@code 2}. One payment or collection. */
    DATA("data"),

    /** トレーラーレコード — データ区分 {@code 8}. Closes a batch with control totals. */
    TRAILER("trailer"),

    /** エンドレコード — データ区分 {@code 9}. Closes the file. */
    END("end"),
    ...
    /**
     * Maps a descriptor's {@code records} key to a kind.
     *
     * @param key the key, for example {@code "header"}
     ...
     * @throws UnsupportedOperationException if called on {@link #MALFORMED},
