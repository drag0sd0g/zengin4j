...
///
/// Immutable and thread-safe (R-T1): build one and share it. There is
/// deliberately no `register` method that mutates an existing registry —
/// [#withFormat(FormatDescriptor)] returns a new instance instead, so a
...
public final class FormatRegistry {
    ...
    private FormatRegistry(Map<FormatId, FormatDescriptor> formats, Map<String, CodeList> codeLists) {
        // Not Map.copyOf: that returns an unordered map, and declaration order is
        // what diagnostics and the generated documentation list things in.
    ...
    ///
    /// Every bundled descriptor is `verified: false` in 0.1.0. Reading
    /// a file with one requires
    /// `ReaderOptions.builder().allowUnverifiedFormats(true)`.
    ///
    /// Nothing is parsed here. The descriptors are authored as YAML and
    ...
    ///
    /// Each call constructs a fresh registry; there is no cached instance,
    ...
    ///
    /// Returns a list rather than an `Optional` because the codes are
    /// not unique: 預金口座振替 and 口座振替結果 share `91`. A caller
    ...
    /// diagnostics.
    ///
    /// @return the registered type codes, or `"none"` if the registry is
    ///   empty
    ...
    ///
    /// Not thread-safe; the registry it builds is.
    ///
    /// @since 0.1.0
