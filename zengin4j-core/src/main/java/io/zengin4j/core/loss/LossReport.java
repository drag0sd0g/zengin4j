 ...
 *
 * <p>Returned alongside the output rather than instead of it: a conversion that
 * lost something still produced a usable result, and the caller decides whether
 * the loss is acceptable. {@link #isLossless()} answers the common question in
 * one call; {@link #atLeast} answers "is any of it serious enough to stop for".
 *
 * <p>Entries arrive in the order the losses happened, which for a field-by-field
...
public final class LossReport {
     ...
     * @return {@code true} if any entry meets it
     */
