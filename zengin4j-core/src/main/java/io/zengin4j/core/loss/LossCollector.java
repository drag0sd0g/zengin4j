...
///
/// Passed into an operation rather than returned from it, so that a
...
///
/// **Not thread-safe**, deliberately. A collector belongs to one
...
public final class LossCollector {
    ...
    ///
    /// One per conversion of one value. Sharing one across threads would
    ...
    ///
    /// @return `true` if no entry has been recorded
    ...
    /// Builds the report.
    ///
    /// The collector stays usable afterwards; the report is a snapshot.
    ///
