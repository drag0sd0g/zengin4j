...
///
/// [#MS932] is the default because it is what Windows-based Japanese
/// accounting systems emit in practice, not because it is the most standard
/// choice. `MS932` and [#SHIFT_JIS] differ in the NEC/IBM extension
/// ranges and in the mapping of the wave dash; the divergence is documented in
/// `docs/encoding.md`.
///
/// **[#UTF_8] breaks the format's central assumption.**
...
public enum ZenginCharset {
    ...
    ///
    /// @return the JDK charset name, never `null`
    ...
    ///
    /// @param buffer the source buffer, never `null`
    /// @param offset start offset within the buffer
    /// @param length number of **bytes** to decode (R-C15)
    ...
    /// @return the encoded bytes, never `null`
    /// @throws CharsetUnavailableException if this JVM does not provide the
    ///   charset
