...
///
/// **Validation returns offsets, not a verdict.** A boolean
/// answers "is this file acceptable", which the caller already suspects is no.
/// The useful question is *which byte*, and a fixed-length format can
...
///
/// Offsets are relative to the start of the array passed in. Validating a
/// whole record yields record-relative offsets; validating one field's slice
/// yields field-relative ones. [#validateField] does the arithmetic for
/// the common case of a field within a record.
///
/// Nothing here allocates when the input is clean, which is the usual case
...
public final class CharacterSet {
    ...
    /// @return the violations, empty if clean; never `null`
    ...
    /// violation list.
    ///
    /// For the caller that only branches on the answer — a strict-mode check
    ...
    /// @return `true` if every byte is permitted
    /// @throws IndexOutOfBoundsException if the range is outside the buffer
    ...
    ///
    /// Record-relative is what a finding needs: it composes with the record's
