...
/// (R-C13).
///
/// Orthogonal to [ParseMode], which governs *structure*. A
/// record can be structurally perfect — right length, right discriminator,
/// numerics that parse — and still carry a name containing `ｰ`, which the
/// receiving institution will reject. The two questions are separate, so the two
/// settings are separate.
///
/// [#IGNORE] is the default, deliberately. Reading is for getting at
...
public enum CharacterPolicy {
    ...
    /// a [io.zengin4j.core.error.CharacterSetViolationException] naming
    /// every violation in that record.
    ///
    /// This is R-C13's strict mode. Use it when producing a file for
