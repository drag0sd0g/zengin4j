...
///
/// **The buffer is recycled. A view is valid only until the next call
/// to [ZenginReader#next()].** Nothing is copied when a view is
...
/// [StaleRecordViewException] rather than returning plausible wrong
/// values.
///
/// To keep a record, call [#materialize()]. To avoid thinking about it
/// at all, use [BatchReader], which materialises by default (R-MEM5).
///
/// Field values are decoded on access and cached for the life of the view
...
public final class RecordView {
    ...
    ///
    /// @return the kind, or [RecordKind#MALFORMED] if the record could
    ///   not be interpreted
    ...
    ///
    /// @return `true` if no layout matched, or the record is truncated
    ...
    ///
    /// @return `false` once the reader has advanced past it
    ...
    ///   [MalformedRecord] if the record could not be interpreted
