...
///
/// Two reading APIs with two contracts. [io.zengin4j.core.codec.ZenginReader]
...
/// [io.zengin4j.core.codec.BatchReader] materialises everything into
/// immutable values, and is the one to reach for unless the profiler says
/// otherwise.
///
/// Writing is the reverse and deliberately not symmetrical.
/// [io.zengin4j.core.codec.ZenginWriters] emits each record from the raw
...
/// equivalent. [io.zengin4j.core.codec.ZenginFileBuilder] is for files that
/// did not come from bytes in the first place; it encodes through
/// [io.zengin4j.core.codec.RecordEncoder] and then materialises through the
/// same path a reader uses, so a built file and a read one are the same kind of
/// thing.
///
/// Record length is a property of the format descriptor, never a constant in
