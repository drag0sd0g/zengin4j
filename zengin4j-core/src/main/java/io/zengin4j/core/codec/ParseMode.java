...
public enum ParseMode {
    ...
    ///
    /// The default. Appropriate when the file is expected to be well formed
    /// and a surprise should halt processing.
    STRICT,

    /// Emit a [io.zengin4j.core.model.MalformedRecord] and carry on,
    /// resynchronising by exactly one record length (R-C3).
    ///
    /// Appropriate for diagnostics: one bad record should not hide the
