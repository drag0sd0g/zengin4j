...
///
/// Records are fixed width, so framing is arithmetic rather than scanning.
/// The only variable is what sits between them: nothing, `CR`,
/// `LF` or `CRLF`, possibly inconsistently within one file (R-C6).
...
public final class RecordFramer {
    ...
    /// `0x1A`, accepted as a trailing end-of-file marker (R-C8).
    ...
    /// @return the position of the first non-separator byte, or `limit`
    ...
    /// Returns the offset at which the record after the one at `position`
    /// begins.
    ///
    /// @param buffer   the buffer
    /// @param position offset of the current record
    /// @param limit    one past the last valid byte
    /// @return the offset of the next record, or `limit` if the buffer
    ///   ends first
    ...
    /// @param limit    one past the last valid byte
    /// @return `true` if the three byte order mark bytes are present
