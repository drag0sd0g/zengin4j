...
/// (R-C16, R-C17).
///
/// Lives here rather than in `core.charset` because it needs the
...
/// [CharacterClass] stays a value anyone can use without a descriptor.
///
/// @since 0.1.0
public final class RecordCharacters {
    ...
    /// permit.
    ///
    /// Offsets are record-relative, so they compose with the record's own
    /// offset to give a position in the file.
    ///
    /// Trailing pad bytes are checked like any other: a `C` field is
    ...
    /// @param descriptor the record's layout
    /// @return `true` if no field carries a byte it may not
