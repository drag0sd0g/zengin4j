...
///
/// The default refuses, because the alternative is the failure §16.2
...
public enum TruncationPolicy {
    ...
    ///
    /// Shortening a payee's name is a decision about somebody's money, and
    ...
    /// As [#TRUNCATE_SAFE], but end the result with a marker so a reader
    /// of the file can see that something was removed.
    ///
    /// The marker costs a byte of the field, so the text is shortened one
    /// byte further than `TRUNCATE_SAFE` would.
