...
/// ```
///
/// **Nothing here happens silently.** Every conversion that
/// changes what a name says produces a [LossEntry], and every conversion
...
///
/// **The target field is an argument, not a detail.** A long
/// vowel becomes a hyphen, and [CharacterClass#PAYROLL_NAME] admits no
...
///
/// Stateless and thread-safe: every method takes what it needs.
///
/// @since 0.4.0
public final class KanaTransliterator {

    /// `ﾞ`, which modifies the kana before it (§16.1).
    ...
    ///
    /// The two steps belong together: how long the text is can only be known
    ...
    ///
    /// Character by character rather than byte by byte, because
    ...
    /// UTF-8 bytes made it look for `0xDE` where the mark's first byte is
    /// `0xEF`: it kept the base and dropped the mark, turning ﾌﾞ into ﾌ.
    ...
    ///
    /// Easy to get wrong, and silent when it goes wrong: the obvious marker
    /// `*` is permitted by *no* name class, so marked truncation
    /// used to produce a shortened name that `V-202` then rejected — a
    /// policy whose whole purpose is to make a change visible, making the file
    /// invalid instead.
    ///
    /// `PAYROLL_NAME` admits no symbol at all, so marked truncation is
    ...
    /// (R-K3, §16.3).
    ///
    /// The hazard is one byte wide. `ｶﾞ` is `0xB6 0xDE`, and a cut
    /// between them leaves `ｶ` — a different character, in a file that
    /// records no sign of the change. So a cut that would land on a mark takes
    /// the base kana with it.
    ///
    /// Operates on **JIS X 0201 bytes**, one per half-width
    ...
    ///
    /// Marked informational because it is not reliably reversible: several
    ...
    ///
    /// @return `true` if anything was narrowed
    ...
    ///
    /// Not a stylistic choice: no field class permits a lowercase letter, so
    ...
    ///
    /// The last pass, and the one that catches the case the others cannot:
    /// every character is now a legal half-width character, and some of them are
    /// still not permitted *here*. A hyphen is fine in a party name and
    ...
    ///
    /// Not a hypothetical. `ヷ` — katakana VA — narrows mechanically to
    /// `ﾜﾞ`, and `ﾜ` has no voiced form, so the mark is stranded.
    /// Unicode is happy to decompose it; the standard has nowhere to put it, and
    /// validation rule `V-206` would report the result. Producing a file
    ...
    /// later.
    ///
    /// Catches half-width input the caller supplied directly, too: a stray
