...
/// Conversion between the four-digit `MMDD` field encoding and
/// [MonthDay].
///
/// @since 0.1.0
public final class MonthDays {

    /// Length in bytes of an `MMDD` field.
    public static final int FIELD_LENGTH = 4;

    private MonthDays() {
    }

    /// Parses an `MMDD` field.
    ///
    /// Returns empty rather than throwing for every unusable value: the
    ...
    /// Renders a month and day as an `MMDD` field value.
    ///
