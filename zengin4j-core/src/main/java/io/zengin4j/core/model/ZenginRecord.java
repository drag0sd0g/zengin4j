 ...
 *
 * <p>The hierarchy is sealed over the five things a record can be, so a
 * {@code switch} over it is exhaustive without a default branch (R-D3). The
 * four role interfaces are {@code non-sealed}: each format contributes its own
 * concrete types, generated from its descriptor, and consumers may add their
 * own for institution-specific variants.
 *
 * <p><strong>The domain model is format-shaped, not idealised</strong> (R-D1).
 * {@code SougouFurikomiHeader} has exactly the fields the 総合振込 header has,
...
public sealed interface ZenginRecord
     ...
     * from.
     *
     * @return the offset, or {@code -1} for a record that was built rather
     *         than read
     */
    long byteOffset();
     ...
     *
     * @return the record number, or {@code 0} for a record that was built
     *         rather than read
     */
    int recordNumber();
     ...
     *
     * <p>Retaining the raw bytes is what makes byte-exact round-tripping
     * possible (R-D5): unknown, reserved and filler bytes are preserved
     * verbatim rather than regenerated from the decoded fields.
     *
     * <p>Returns a fresh copy on every call, so the record stays immutable.
     *
