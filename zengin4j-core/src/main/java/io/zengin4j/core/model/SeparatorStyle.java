...
///
/// Separators are optional in these formats and are not counted in the
...
public enum SeparatorStyle {
    ...
    /// Carriage return only, `0x0D`.
    CR(new byte[] {'\r'}),

    /// Line feed only, `0x0A`.
    LF(new byte[] {'\n'}),

    /// Carriage return and line feed, `0x0D 0x0A`. The default on write (R-C9).
    CRLF(new byte[] {'\r', '\n'}),

    /// More than one convention within a single file.
    ///
    /// Such a file cannot be reproduced byte for byte from a single
    ...
    ///
    /// @return a copy of the separator bytes, or empty for [#MIXED],
