 ...
 *
 * <p>The Japanese encodings this library uses live in the {@code jdk.charsets}
 * module. A JVM assembled with {@code jlink} that omits it will decode nothing
 * useful, so the condition is reported by name at first use rather than
 * surfacing as an obscure {@code UnsupportedCharsetException} from deep inside
 * a decode loop.
 *
 * @since 0.1.0
 */
public final class CharsetUnavailableException extends ZenginException {
     ...
     * @param cause       the underlying lookup failure, may be {@code null}
     */
     ...
     *
     * @return the charset name, never {@code null}
     */
