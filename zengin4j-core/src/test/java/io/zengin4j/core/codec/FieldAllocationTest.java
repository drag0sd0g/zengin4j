...
/// value.
///
/// The claim appears in several places in this codebase — the record view's
...
///
/// **The measurement is a difference, not an absolute.** Total
...
/// fields *read*. So this decodes every numeric field of every record,
...
///
/// Uses `com.sun.management.ThreadMXBean`, which HotSpot provides and
/// other JVMs may not; the test skips rather than fails where it is absent.
///
/// **These pass under `-Xint`**, with the JIT disabled
/// entirely. That matters: escape analysis will remove a short-lived
/// `Optional` or iterator once a path is hot, which makes it easy to
...
class FieldAllocationTest {
    ...
    ///
    /// Generous on purpose. What is being distinguished is "does not scale"
    ...
    /// Reads every record, decoding the first `fields` numeric fields of each.
